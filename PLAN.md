# Kế hoạch triển khai dự án: Ứng dụng hình học của tích phân (2 tuần)

## Tuần 1: Nghiên cứu, soạn thảo lý thuyết

### Tuần 1 - Giai đoạn 1: Đọc tài liệu, chuẩn bị nội dung lý thuyết (3 ngày)
- [ ] Tìm hiểu và tạo một dự án trong Github.
- [ ] Tìm hiểu về LaTeX.
- [ ] Tìm hiểu, đọc các tài liệu về dự án.
- [ ] Viết mô tả chi tiết cho tệp `README.md`.

### Tuần 1 - Giai đoạn 2: Bài tập & Minh họa TikZ (4 ngày)
- [ ] Soạn thảo phần mở đầu (lý do chọn đề tài).
- [ ] Soạn thảo nội dung chương I.
- [ ] Soạn thảo nội dung chương II.
- [ ] Sưu tầm, phân loại và soạn phương pháp giải, lời giải cho các bài tập điển hình (Tính diện tích, thể tích, độ dài cung).
- [ ] Lập trình biểu đồ và hình vẽ minh họa bằng TikZ/PGFPlots (mặt tròn xoay, hình phẳng giới hạn bởi đồ thị hàm số, hình ảnh lát cắt).
- [ ] Review và hoàn thiện nội dung lý thuyết + bài tập Tuần 1.

---

## Tuần 2: Tối ưu hóa, kiểm tra & xuất bản

### Tuần 2 - Giai đoạn 1: Đóng gói & rà soát lỗi (4 ngày)
- [ ] Soạn thảo kết luận và tổng kết dự án.
- [ ] Biên dịch toàn bộ tài liệu LaTeX, kiểm tra và sửa toàn bộ lỗi biên dịch (Warnings/Errors).
- [ ] Tối ưu hóa định dạng, bố cục trang, font chữ và căn chỉnh vị trí các hình vẽ TikZ.

### Tuần 2 - Giai đoạn 2: Quản lý mã nguồn & Nghiệm thu (3 ngày)
- [ ] Dọn dẹp repository (Xóa repo cũ nếu cần, tạo repo mới chuyên nghiệp).
- [ ] Đồng bộ hóa toàn bộ mã nguồn LaTeX, tệp `.tex`, `.sty` và hình ảnh lên GitHub.
- [ ] Xuất tệp PDF chất lượng cao chính thức phục vụ nộp bài và giảng dạy.

## Cập nhật tiến độ mới nhất (18/09/2026)

### Đã hoàn thành
- [x] Thiết kế trang bìa chuẩn theo biểu mẫu và phần lí do làm dự án
- [x] Xây dựng cấu trúc mô-đun tệp LaTeX cho toàn bộ tài liệu
- [x] Soạn thảo nội dung lý thuyết chương I.
- [x] Soạn thảo nội dung lý thuyết chương II.
- [x] Vẽ và tối ưu hóa các hình minh họa, đồ thị bằng công cụ TikZ/pgfplots (đồ thị hàm số, hàm lượng giác, ý nghĩa hình học tích phân)
- [x] Hoàn thiện phần kết luận và tổng kết dự án

### Vấn đề cần giải quyết & Kế hoạch tiếp theo
- [ ] Kiểm tra lỗi biên dịch toàn bộ tệp.
- [ ] Xóa repository cũ và tạo mới, đồng bộ mã nguồn lên GitHub
- [ ] Xuất tệp PDF chất lượng cao phục vụ nộp bài tập và giảng dạy
